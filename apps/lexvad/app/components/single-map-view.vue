	...
	<div class="relative flex flex-col gap-5">
			...
			<div class="grow rounded-lg border p-5 max-w-full">
				...
				<div class="mt-5 flex justify-between">
					<div class="flex gap-2 items-center">
						...
						<UTabs v-model="mapMode" class="w-fit" color="neutral" :content="false" :items="mapModeItems"
							:ui="{
		...
		<VisualisationContainer v-slot="{ height, width }" class="border h-[600px]" :fullscreen="false">
			...
			<div v-if="height && width" class="w-full h-full">
				<GeoMap :colors="uniqueVariants.map((v) => getColorForVariant(activeQuestion ?? '', v.anno) ?? '')
					" :data="data" :mode="mapMode">
				</GeoMap>
			</div>
