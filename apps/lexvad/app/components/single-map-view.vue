...
const mappedQuestions = computed(() => {
	return [...new Set(data.map((entry) => entry.Item))].map((q) => ({ label: q, value: q }));
});
const uniqueVariants = computed(() => {
	const allCurrentVariants = data
		.filter((entry) => entry.Item === activeQuestion.value)
		.flatMap((entry) => entry.Benennungsvariante.split("; "));
	const variantSet = new Set(allCurrentVariants);
	return [...variantSet.keys()]
		.map((v) => ({
			anno: v,
			value: v,
			label: v,
			count: allCurrentVariants.filter((variant) => variant === v).length,
		}))
		...
		<div class="flex gap-2">
			<div class="grow rounded-lg border p-5 max-w-full">
				<div class="flex gap-5 pb-5 border-b border-muted max-w-full flex-wrap">
					...
					<template v-if="!splitMode">
						...
						<div class="gap-2 flex ml-auto self-end">
							...
							<Button class="p-2 gap-2" variant="outline" @click="emit('toggle-sidebar')">
								<InfoIcon class="size-4"></InfoIcon>
				...
				<div class="mt-5 flex justify-between">
					<ToggleGroup
						v-model="mapMode"
						class="gap-0 p-1 bg-accent w-fit rounded-lg"
						type="single"
						variant="outline"
					>
						<ToggleGroupItem
