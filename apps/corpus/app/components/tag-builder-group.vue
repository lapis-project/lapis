...
const featsQuery = computed(() => {
	return builders.value
		...
		.reduce((acc, clause, index) => {
			...
			const op = logic === "AND" ? " & " : logic === "OR" ? " | " : logic === "NOT" ? " ! " : " & ";
					...
					<div v-if="activeBuilderId === builder.id && tagSelected">
						<Popover>
							...
							<PopoverContent align="start" class="w-fit flex flex-row gap-1 p-1">
								...
								<Button class="justify-start" variant="ghost" @click="addBuilder('OR')">
									OR
								</Button>
			...
			<div class="flex flex-col min-h-0">
				...
				>
					<div class="p-4 border shadow-sm">
						<Button class="w-full" @click="executeSearch"> Suchen </Button>
					</div>
				</div>
			</div>
		</div>
