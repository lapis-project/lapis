            ...
            <div class="grow rounded-lg border p-5 max-w-full">
                <div class="flex gap-5 pb-5 border-b border-muted max-w-full flex-wrap">
                    <div id="phenomenon" class="w-full flex-1">
                        ...
                        <ComboboxBase
v-if="mappedQuestions?.length" v-model="activeQuestion" data-testid="questions"
...
v-model="activeVariants" data-testid="variants" :options="uniqueVariants"
                    ...
                    <template v-if="!splitMode">
                        ...
                        <div class="gap-2 flex ml-auto self-end">
                            ...
                            <Button class="p-2 gap-2" variant="outline" @click="emit('toggle-sidebar', activeQuestion)">
                                <InfoIcon class="size-4"></InfoIcon>
                                <span>{{ t("MapsPage.controls.open-sidebar") }}</span>
                            </Button>
                            <Button
id="reset" data-testid="reset" size="icon" variant="outline"
                ...
                <div class="mt-5 flex justify-between">
                    <ToggleGroup
v-model="mapMode" class="gap-0 p-1 bg-card w-fit rounded-lg" type="single"
                        variant="outline">
                        <ToggleGroupItem
...
id="reset" data-testid="reset" size="icon" variant="outline"
        ...
        <VisualisationContainer class="border h-[600px]" :fullscreen="false">
            <div
v-if="uniqueVariants.length" id="variantLegend" class="absolute bottom-4 right-0 z-10 mr-4"
                data-testid="variantLegend">
                <div class="rounded-md border border-input bg-background p-3 text-sm text-foreground shadow-md">
                    <ul class="space-y-0.5">
                        <li v-for="variant in uniqueVariants" :key="variant.anno" class="flex items-center gap-1">
                            <span>{{ variant.anno }}</span>({{ variant.count }})
                        </li>
                    </ul>
                </div>
            </div>
